...
prefs = {"translate_whitelists": {"ru": "en"}, "translate": {"enabled": "true"}}
        ...
        if answer == "Yes":
            ...
            if select == "Libgen":
                ...
                sub_select = questionary.select(
                    "Select item",
                    choices=["Search by Title", "Search by Author", "Exit"],
                ).ask()
                ...
                if sub_select == "Search by Title":
                    choice = 1
                    Booksources().Annas_Archive().annas_archive_search(
                        search_term, choice
                    )
                elif sub_select == "Search by Author":
                    choice = 2
                ...
                sub_select = questionary.select(
                    "Select item",
                    choices=[
                        "Search General Courses",
                        "Search Computer Courses",
                        "Exit",
                    ],
                ).ask()
